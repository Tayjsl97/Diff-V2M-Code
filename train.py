...
class PrintGlobalStepCallback(pl.Callback):
...
def setup():
    ...
    print(f"Process {os.getpid()} using GPU {torch.cuda.current_device()}")
...
def main():
    ...
    my_print("checkpoint_dir: "+str(checkpoint_dir))
    ...
    try:
        ...
        model_config["training"]["demo"]['experiment_id'] = logger.experiment.id
        if args.val_dataset_config:
            demo_callback = create_demo_callback_from_config(model_config, demo_dl=val_dl)
        else:
            demo_callback = create_demo_callback_from_config(model_config, demo_dl=train_dl)
    except Exception as e:
        print(f"Error creating demo callback: {e}")
