...
def get_video_filenames(
    ...
    "recursively get a list of video filenames"
    filenames = []
    if type(paths) is str:
        paths = [paths]
    for path in paths:               # get a list of relevant filenames
...
def get_video_info(
    ...
    "recursively get a list of video filenames"
    videonames = [os.path.splitext(os.path.basename(path))[0] + '.mp4' for path in video_files]
    scene_cut = []
    pseudo_beat = []
    # video_cap = []
    with open(paths, 'r', encoding='utf-8') as f:
        video_info = json.load(f)
    for videoname in videonames:               # get a list of relevant filenames
        scene_cut.append(video_info[videoname]['scene_cut'])
        pseudo_beat.append(video_info[videoname]['pseudo_beats'])
        # video_cap.append(video_info[videoname]['video_cap'])
    return scene_cut,pseudo_beat
        ...
        try:
            ...
            for root_path in self.root_paths:
                if root_path in audio_filename:
                    ...
                    info["prompt"] = path.relpath(audio_filename, root_path)
...
class AVDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.duration = sample_size//sample_rate

        self.custom_metadata_fns = {}

        for config in configs:
            self.root_paths.append(config.path)
            audio_files = sorted(get_audio_filenames(config.path, keywords), key=lambda x: Path(x).stem)
            video_files = sorted(get_video_filenames(config.video_feat, keywords), key=lambda x: Path(x).stem)
            assert len(audio_files) == len(
                video_files), f"Mismatched audio/video count: {len(audio_files)} vs {len(video_files)}"
            for a, v in zip(audio_files, video_files):
                assert Path(a).stem == Path(v).stem, f"Mismatch: {a} vs {v}"
                self.filenames.append(a)
                self.video_filenames.append(v)
            if config.video_color is not None:
                color_files = sorted(get_video_filenames(config.video_color, keywords), key=lambda x: Path(x).stem)
                self.color_filenames.extend(color_files)
            if config.video_info is not None:
                scene_cut,pseudo_beat=get_video_info(config.video_info, video_files, keywords)
                self.scene_cut.extend(scene_cut)
                self.pseudo_beat.extend(pseudo_beat)
                # self.video_cap.extend(video_cap)
            self.class_label.extend([config.class_label]*len(get_audio_filenames(config.path, keywords)))
            if config.custom_metadata_fn is not None:
                self.custom_metadata_fns[config.path] = config.custom_metadata_fn

        print(f'Found audio {len(self.filenames)} files')
        print(f'Found video {len(self.video_filenames)} files')
    ...
    def load_file(self, filename):
        ...
        try:
            ...
            print(f'Couldn\'t load audiofile {filename}: {e}'"")
        ...
        # print("read audio succeed!: ", filename)
        return audio

    def extract_mel_spectrogram(self,wav_data, sr=22050, n_mels=128, hop_length=512, n_fft=2048):
        ...
        mel_spectrogram = mel_spectrogram_transform(wav_data)
        log_mel_spectrogram = T.AmplitudeToDB()(mel_spectrogram)
        return log_mel_spectrogram

    def extract_tempogram(self,wav_data, sr=22050, hop_length=128, win_length=384):
        if isinstance(wav_data, torch.Tensor):
            wav_data = wav_data.detach().cpu().numpy()
        tempogram = librosa.feature.tempogram(y=wav_data, sr=sr, hop_length=hop_length, win_length=win_length)
        return torch.from_numpy(tempogram).float()
    
    def extract_ODF(self,wav_data, sr=22050, hop_length=512):
        if isinstance(wav_data, torch.Tensor):
            wav_data = wav_data.detach().cpu().numpy()
        odf = librosa.onset.onset_strength(y=wav_data, sr=sr, hop_length=hop_length)
        mean = odf.mean()
        std = odf.std() + 1e-6
        odf = (odf - mean) / std
        frame_times = librosa.frames_to_time(np.arange(len(odf)), sr=sr, hop_length=hop_length)
        duration = librosa.get_duration(y=wav_data, sr=sr)
        num_seconds = int(np.ceil(duration))
        peaks, _ = find_peaks(odf, prominence=0.05)
        peak_times = frame_times[peaks]
        peak_values = odf[peaks]
        odf_per_sec = np.zeros(num_seconds)
        for t, val in zip(peak_times, peak_values):
            sec = int(round(t))
            if sec < num_seconds:
                odf_per_sec[sec] = max(odf_per_sec[sec], val)  # 多个峰取最大值
        odf_per_sec = torch.from_numpy(odf_per_sec).unsqueeze(-1).float()
        if odf_per_sec.shape[0]>30:
            odf_per_sec=odf_per_sec[1:]
        return odf_per_sec

    def extract_rhythm(self,y,sr,rhythm_type):
        if y.shape[0]==2:
            y = y.mean(dim=0,keepdims=False)
        if rhythm_type == "mel":
            mel_spectrogram = self.extract_mel_spectrogram(y, sr)
            return mel_spectrogram.T
        elif rhythm_type == "tempo":
            tempogram = self.extract_tempogram(y, sr)
            return tempogram.T
        elif rhythm_type == "odf":
            odf = self.extract_ODF(y, sr)
            return odf
        else:
            return None
    ...
    def __getitem__(self, idx):
        ...
        if len(self.color_filenames)>0:
            color_filename = self.color_filenames[idx]
        class_label = self.class_label[idx]
        if len(self.scene_cut) > 0:
            scene_cut = self.scene_cut[idx]
            pseudo_beat = self.pseudo_beat[idx]

        start_time = time.time()
        audio = self.load_file(audio_filename)
        rhythm = self.extract_rhythm(audio,self.sr,self.rhythm_type)
        video = np.load(video_filename)
        if len(self.color_filenames) > 0:
            color = np.load(color_filename)
        ...
        if len(self.scene_cut) > 0:
            ...
            info['pseudo_beat'] = pseudo_beat
        if self.rhythm_type == "beat" and rhythm is None:
            rhythm=np.array(scene_cut)+np.array(pseudo_beat)
            rhythm=torch.from_numpy(rhythm).float().unsqueeze(-1)
        info['rhythm'] = rhythm
        info["timestamps"] = (t_start, t_end)
        info["seconds_start"] = seconds_start
        info["seconds_total"] = seconds_total
        info["padding_mask"] = padding_mask
        info["sample_rate"] = self.sr
        info['rhythm_type'] = self.rhythm_type
...
def create_dataloader_from_config(dataset_config, batch_size, sample_size, sample_rate, audio_channels=2, num_workers=4, shuffle = True):
    ...
    rhythm_type = dataset_config.get("rhythm_type", None)
    ...
    if dataset_type == "audio_dir":
        ...
        return torch.utils.data.DataLoader(train_set, batch_size, shuffle=shuffle,num_workers=num_workers,
                                           persistent_workers=persistent_workers, pin_memory=True,
                                         drop_last=dataset_config.get("drop_last", True), collate_fn=collation_fn)

    elif dataset_type == "audio_video_dir":
        audio_video_dir_configs = dataset_config.get("datasets", None)

        assert audio_video_dir_configs is not None, "Directory configuration must be specified in datasets[\"dataset\"]"

        configs = []

        for audio_video_dir_config in audio_video_dir_configs:
            audio_dir_path = audio_video_dir_config.get("path", None)
            video_dir_path = audio_video_dir_config.get("video_feat", None)
            color_dir_path = audio_video_dir_config.get("video_color", None)
            video_info_path = audio_video_dir_config.get("video_info", None)
            class_label = audio_video_dir_config.get("class_label", None)
            assert audio_dir_path is not None, "Path must be set for local audio directory configuration"
            assert video_dir_path is not None, "Path must be set for local video feat directory configuration"

            custom_metadata_fn = None
            custom_metadata_module_path = audio_video_dir_config.get("custom_metadata_module", None)
            ...
            configs.append(
                LocalDatasetConfig(
                    id=audio_video_dir_config["id"],
                    path=audio_dir_path,
                    video_feat=video_dir_path,
                    video_info=video_info_path,
                    video_color=color_dir_path,
        ...
        if num_workers==0:
            ...
            persistent_workers=True
        dd=dataset_config.get("drop_last", True)
        print(f"drop_last: {dd}")
