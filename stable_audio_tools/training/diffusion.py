...
class DiffusionCondTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        ...
        self.loss_modules = [
            ...
            MSELoss("predict_rhythm",
                    "real_rhythm",
                    weight=1.0,
                    mask_key="rhythm_mask",
                    name="rhythm_loss")
        ]
    ...
    def training_step(self, batch, batch_idx):
        ...
        class_label=torch.tensor(class_list).long().to(self.device)
        ...
        output = self.diffusion(noised_inputs, t, self.trainer.current_epoch, class_label, \
                                cond=conditioning, cfg_dropout_prob=self.cfg_dropout_prob, **extra_args)
        p.tick("diffusion")
        
        if 'fake_rhythm_loss' in conditioning.keys():
            loss_info.update({
                "output": output,
                "targets": targets,
                "padding_mask": padding_masks if use_padding_mask else None,
                "predict_rhythm":conditioning['fake_rhythm_loss'],
                "real_rhythm":conditioning['real_rhythm_loss'],
                "rhythm_mask":conditioning['rhythm'][1].long()
            })
        else:
            loss_info.update({
                "output": output,
        ...
        if "experiment_id" in self.demo_config:
            experiment_id = self.demo_config["experiment_id"]
        else:
            experiment_id = "temptemp"
        if isinstance(experiment_id, str):
            if not os.path.exists(f"figures/{experiment_id}"):
                os.makedirs(f"figures/{experiment_id}")

            if self.global_step%100==0 and 'fake_rhythm_loss' in conditioning.keys():
                plt.imshow(conditioning['real_rhythm_loss'][0].detach().cpu().numpy().T, aspect='auto', origin='lower', cmap='inferno')
                plt.colorbar(format='%+2.0f dB')
                plt.savefig(f"figures/{experiment_id}/real_{self.global_step}.png")
                plt.clf()
                plt.imshow(conditioning['fake_rhythm_loss'][0].detach().cpu().numpy().T, aspect='auto', origin='lower', cmap='inferno')
                plt.colorbar(format='%+2.0f dB')
                plt.savefig(f"figures/{experiment_id}/fake_{self.global_step}.png")
                plt.clf()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for validation_timestep in self.validation_timesteps:
            ...
            with torch.cuda.amp.autocast() and torch.no_grad():
                output = self.diffusion(noised_inputs, t, self.trainer.current_epoch, class_label, cond=conditioning, cfg_dropout_prob = 0, **extra_args)

                val_loss = F.mse_loss(output, targets)
                val_aver_loss += val_loss.item()
                
                if 'fake_rhythm_loss' in conditioning.keys():
                    rhythm_loss = F.mse_loss(conditioning['fake_rhythm_loss'], conditioning['real_rhythm_loss'], reduction='none')*conditioning['rhythm'][1].unsqueeze(-1).long()
                    denominator = conditioning['rhythm'][1].long().sum()
                    if denominator > 0:
                        val_rhythm_loss += (rhythm_loss.sum()/denominator).item()
                    else:
                        val_rhythm_loss += (rhythm_loss.sum().item())

                    self.validation_step_outputs[f'val/loss_{validation_timestep:.1f}'].append(val_loss.item())
        
        if 'fake_rhythm_loss' in conditioning.keys():
            log_dict = {
                "val_loss":val_aver_loss/len(self.validation_timesteps),
                "val_rhythm_loss":val_rhythm_loss/len(self.validation_timesteps),
            }
        else:
            log_dict = {
                "val_loss":val_aver_loss/len(self.validation_timesteps) 
    ...
    def on_train_batch_end(self, trainer, module: DiffusionCondTrainingWrapper, outputs, batch, batch_idx):
        ...
        if self.demo_cond_from_batch:
            ...
            for one in demo_cond:
                class_list.append(one["class"])
            class_label = torch.tensor(class_list).long().to(module.device)
            ...
            for cfg_scale in self.demo_cfg_scales:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    if module.diffusion_objective == "v":
                        fakes = sample(model, noise, self.demo_steps, 0, class_label, **cond_inputs, cfg_scale=cfg_scale, dist_shift=module.diffusion.dist_shift, batch_cfg=True)
                ...
                demo_save_path=os.path.join(self.demo_save_dir,self.experiment_id)

                if not os.path.exists(demo_save_path):
                    os.makedirs(demo_save_path)

                filename = f'{demo_save_path}/demo_cfg_{cfg_scale}_{trainer.global_step:08}.wav'
