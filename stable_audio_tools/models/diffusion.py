...
class ConditionedDiffusionModel(nn.Module):
    ...
    def forward(self,
                ...
                cross_attn_cond: tp.List[torch.Tensor] = [],
                cross_attn_mask: tp.List[torch.Tensor] = [],
...
class ConditionedDiffusionModelWrapper(nn.Module):
    ...
    def get_conditioning_inputs(self, conditioning_tensors: tp.Dict[str, tp.Any], negative=False):
        ...
        if len(self.cross_attn_cond_ids) > 0:
            ...
            for key in self.cross_attn_cond_ids:
                ...
                if key == "semantic":
                    cross_semantic.append(cross_attn_in)
                    mask_semantic.append(cross_attn_mask)
                elif key == "rhythm":
                    cross_rhythm.append(cross_attn_in)
                    mask_rhythm.append(cross_attn_mask)
                    fake_rhythm_in,fake_rhythm_mask = conditioning_tensors["fake_rhythm"]
                    cross_fake_rhythm.append(fake_rhythm_in)
                    mask_fake_rhythm.append(fake_rhythm_mask)
                elif key == "color":
                    cross_color.append(cross_attn_in)
                    mask_color.append(cross_attn_mask)
                else:
                    cross_semantic.append(cross_attn_in)
                    mask_semantic.append(cross_attn_mask)
                    cross_rhythm.append(cross_attn_in)
                    mask_rhythm.append(cross_attn_mask)
                    cross_fake_rhythm.append(cross_attn_in)
                    mask_fake_rhythm.append(cross_attn_mask)
            cross_semantic=torch.cat(cross_semantic, dim=1)
            cross_rhythm=torch.cat(cross_rhythm, dim=1)
            cross_fake_rhythm=torch.cat(cross_fake_rhythm, dim=1)
            mask_semantic=torch.cat(mask_semantic, dim=1)
            mask_rhythm=torch.cat(mask_rhythm, dim=1)
            mask_fake_rhythm=torch.cat(mask_fake_rhythm, dim=1)
            if 'color' in self.cross_attn_cond_ids:
                cross_color = torch.cat(cross_color, dim=1)
                mask_color=torch.cat(mask_color, dim=1)

                cross_attention_input.extend([cross_semantic,cross_rhythm,cross_fake_rhythm,cross_color])
                cross_attention_masks.extend([mask_semantic,mask_rhythm,mask_fake_rhythm,mask_color])
            else:
                cross_attention_input.extend([cross_semantic, cross_rhythm, cross_fake_rhythm])
                cross_attention_masks.extend([mask_semantic, mask_rhythm, mask_fake_rhythm])
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, epoch: int, class_label: torch.Tensor, cond: tp.Dict[str, tp.Any], **kwargs):
        return self.model(x, t, epoch, class_label, **self.get_conditioning_inputs(cond), **kwargs)
...
class DiTWrapper(ConditionedDiffusionModel):
    def __init__(
        ...
        **kwargs
        ...
        self.last_printed_epoch_train = -1
        self.last_printed_epoch_test = -1
    ...
    def forward(self,
        ...
        if self.training:
            if epoch<10:
                ...
                p_real = 1 - (epoch - 10) / 20
            else:
                p_real = 0
            if epoch != self.last_printed_epoch_train:
                print(f"Training epoch {epoch}, p_real {p_real}")
                self.last_printed_epoch_train = epoch
        else:
            p_real = 0
            if epoch != self.last_printed_epoch_test:
                print(f"Testing epoch {epoch}, p_real {p_real}")
                self.last_printed_epoch_test = epoch
        try:
            batch_size, seq_len, dim = cross_attn_cond[1].shape
            rand_vals = torch.rand(batch_size)
            mask = rand_vals > p_real
            mask = mask.to(cross_attn_cond[1].device)
            cross_attn_cond_selected = torch.where(
                mask[:, None, None],
                cross_attn_cond[2],
                cross_attn_cond[1]
            )
            cross_attn_mask_selected = torch.where(
                mask[:, None],
                cross_attn_mask[2],
                cross_attn_mask[1]
            )
            cross_attn_cond_origin=cross_attn_cond
            cross_attn_cond=cross_attn_cond[:1]
            cross_attn_mask=cross_attn_mask[:1]
            cross_attn_cond.append(cross_attn_cond_selected)
            cross_attn_mask.append(cross_attn_mask_selected)
            assert len(cross_attn_cond) == 2
            assert len(cross_attn_mask) == 2
            if len(cross_attn_cond_origin)==4:
                cross_attn_cond.append(cross_attn_cond[-1])
                cross_attn_mask.append(cross_attn_mask[-1])
                assert len(cross_attn_cond)==3
                assert len(cross_attn_mask)==3
        except Exception as e:
            print("select real or fake rhythm condition error: ",e)
