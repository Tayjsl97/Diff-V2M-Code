...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        if cross_attend:
            ...
            self.cross_attn_scale0 = LayerScale(dim) if layer_scale else nn.Identity()
            ...
            self.cross_attn_scale1 = LayerScale(dim) if layer_scale else nn.Identity()
            ...
            self.cross_attn_scale2 = LayerScale(dim) if layer_scale else nn.Identity()
            if "preFiLM" in self.attention_weight_type:
                self.t_linear = nn.Linear(1,64)
                self.FiLM_gamma_sem = nn.Sequential(
                    nn.Linear(64, 256),
                    nn.ReLU(),
                    nn.Linear(256, dim_context),
                    nn.Sigmoid()
                )
                self.FiLM_gamma_rhy = nn.Sequential(
                    nn.Linear(64, 256),
                    nn.ReLU(),
                    nn.Linear(256, dim_context),
                    nn.Sigmoid()
                )
                self.FiLM_beta_sem = nn.Sequential(
                    nn.Linear(64, 256),
                    nn.ReLU(),
                    nn.Linear(256, dim_context)
                )
                self.FiLM_beta_rhy = nn.Sequential(
                    nn.Linear(64, 256),
                    nn.ReLU(),
                    nn.Linear(256, dim_context)
                )
            elif self.attention_weight_type == "FiLM" or self.attention_weight_type == "FiLM_choose":
                self.t_linear = nn.Linear(1,64)
                self.FiLM_gamma_sem = nn.Sequential(
                    nn.Linear(64, 256),
                    nn.ReLU(),
            ...
            if "gate" in self.attention_weight_type:
                self.gate = nn.Sequential(
                    nn.Linear(1, dim_context),
                    nn.SiLU(),
                    nn.Linear(dim_context, 1),
                    nn.Sigmoid()
                )
            self.color_norm = LayerNorm(dim, **norm_kwargs) if not remove_norms else nn.Identity()
    ...
    def forward(
        ...
        if self.global_cond_dim is not None and self.global_cond_dim > 0 and global_cond is not None:
            ...
            if context is not None and self.cross_attend:
                ...
                semantic_attn = self.cross_attn_scale1(self.cross_attn1(x, context = context, context_mask = context_mask))
                rhythm_attn = self.cross_attn_scale2(self.cross_attn2(x, context = context, context_mask = context_mask))
                attn = 0.5 * semantic_attn + 0.5 * rhythm_attn
                x = x + attn
            ...
            if context is not None and self.cross_attend:
                x = self.cross_attend_norm(x)
                if len(context)==3 and self.attention_weight_type!="only_clip":
                    attn = self.cross_attn_scale0(self.cross_attn0(x, context=context[2], context_mask=context_mask[2]))
                    x += attn
                    x = self.color_norm(x)
                if "FiLM" in self.attention_weight_type:
                    t_embed=self.t_linear(t.unsqueeze(-1))
                    attn_gamma_sem = self.FiLM_gamma_sem(t_embed).view(t.shape[0], 1, -1)
                    attn_gamma_rhy = self.FiLM_gamma_rhy(t_embed).view(t.shape[0], 1, -1)
                    attn_beta_sem = self.FiLM_beta_sem(t_embed).view(t.shape[0], 1, -1)
                    attn_beta_rhy = self.FiLM_beta_rhy(t_embed).view(t.shape[0], 1, -1)
                    if "preFiLM" in self.attention_weight_type:
                        context[0] = context[0] * attn_gamma_sem + attn_beta_sem
                        context[1] = context[1] * attn_gamma_rhy + attn_beta_rhy
                semantic_attn = self.cross_attn_scale1(self.cross_attn1(x, context=context[0], context_mask=context_mask[0]))
                rhythm_attn = self.cross_attn_scale2(self.cross_attn2(x, context=context[1], context_mask=context_mask[1]))
                if t==None:
                    attn=(semantic_attn+rhythm_attn)/2
                else:
                    assert t!=None
                    if self.attention_weight_type == "FiLM_choose":
                        semantic_attn = semantic_attn * attn_gamma_sem + attn_beta_sem
                        rhythm_attn = rhythm_attn * attn_gamma_rhy + attn_beta_rhy
                        attn = torch.where((t > 0.2).view(-1, 1, 1), semantic_attn, rhythm_attn)
                    elif self.attention_weight_type == "FiLM":
                        semantic_attn = semantic_attn * attn_gamma_sem + attn_beta_sem
                        rhythm_attn = rhythm_attn * attn_gamma_rhy + attn_beta_rhy
                        attn = semantic_attn + rhythm_attn
                    elif self.attention_weight_type == "none" or self.attention_weight_type=="only_clip":
                        attn = semantic_attn
                    elif self.attention_weight_type == "choose":
                        attn = torch.where((t > 0.2).view(-1, 1, 1), semantic_attn, rhythm_attn)
                    elif "gate" in self.attention_weight_type:
                        alpha = self.gate(t.unsqueeze(-1)).unsqueeze(-1)
                        attn = alpha * semantic_attn + (1 - alpha) * rhythm_attn
                    else:
                        attn = 0.5 * semantic_attn + 0.5 * rhythm_attn
                x = x + attn
    ...
    def forward(
        ...
        **kwargs
        ...
        for layer in self.layers:
            ...
            x = checkpoint(layer, x, t, class_label, rotary_pos_emb = rotary_pos_emb, global_cond=global_cond, **kwargs)
...
class ExtractorTransformer(nn.Module):
    ...
    def forward(
        ...
        # Iterate over the transformer layers
        for layer in self.layers:
            x = checkpoint(layer, x, t=None, rotary_pos_emb=rotary_pos_emb, **kwargs)
