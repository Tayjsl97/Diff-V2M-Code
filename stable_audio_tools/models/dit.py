...
class DiffusionTransformer(nn.Module):
    def __init__(self, 
        ...
        attention_weight_type: tp.Literal["sigmoid", "linear"] = "sigmoid",
    ...
    def _forward(
        ...
        **kwargs):
        ...
        if cross_attn_cond is not None:
            if isinstance(cross_attn_cond, torch.Tensor):
                cross_attn_cond = self.to_cond_embed(cross_attn_cond)
            if isinstance(cross_attn_cond, list):
                cross_attn_cond = [self.to_cond_embed(i) for i in cross_attn_cond]
        ...
        if self.transformer_type == "x-transformers":
            ...
            output = self.transformer(x, t, class_label, prepend_embeds=prepend_inputs, context=cross_attn_cond, context_mask=cross_attn_cond_mask, mask=mask, prepend_mask=prepend_mask, return_info=return_info, **extra_args, **kwargs)
    ...
    def forward(
        ...
        **kwargs):
        ...
        if cross_attn_cond is not None:
            ...
            if isinstance(cross_attn_cond, list):
                cross_attn_cond = [i.to(model_dtype) for i in cross_attn_cond]
        ...
        if cross_attn_cond_mask is not None:
            if isinstance(cross_attn_cond_mask, torch.Tensor):
                cross_attn_cond_mask = cross_attn_cond_mask.bool()
            if isinstance(cross_attn_cond_mask, list):
                cross_attn_cond_mask = [i.bool() for i in cross_attn_cond_mask]
        ...
        if cfg_dropout_prob > 0.0:
            if cross_attn_cond is not None:
                ...
                if isinstance(cross_attn_cond, list):
                    for i in range(len(cross_attn_cond)):
                        null_embed = torch.zeros_like(cross_attn_cond[i], device=cross_attn_cond[i].device)
                        dropout_mask = torch.bernoulli(torch.full((cross_attn_cond[i].shape[0], 1, 1), cfg_dropout_prob,
                                                                  device=cross_attn_cond[i].device)).to(torch.bool)
                        cross_attn_cond[i] = torch.where(dropout_mask, null_embed, cross_attn_cond[i])
        ...
        if cfg_scale != 1.0 and (cross_attn_cond is not None or prepend_cond is not None) and (cfg_interval[0] <= sigma <= cfg_interval[1]):
            ...
            if cross_attn_cond is not None:
                if isinstance(cross_attn_cond, torch.Tensor):
                    ...
                    null_embed=[torch.zeros_like(i, device=i.device) for i in cross_attn_cond]
                    # for i in range(len(cross_attn_cond)):
                    #     null_embed.append(torch.zeros_like(cross_attn_cond[i], device=cross_attn_cond[0].device))
                ...
                if negative_cross_attn_cond is not None:
                    ...
                    if isinstance(cross_attn_cond, list):
                        batch_cond = [torch.cat([cross_attn_cond[i], null_embed[i]], dim=0) for i in range(len(cross_attn_cond))]

                if cross_attn_cond_mask is not None:
                    if isinstance(cross_attn_cond_mask, torch.Tensor):
                        batch_cond_masks = torch.cat([cross_attn_cond_mask, cross_attn_cond_mask], dim=0)
                    if isinstance(cross_attn_cond_mask, list):
                        batch_cond_masks = [torch.cat([i, i], dim=0) for i in cross_attn_cond_mask]
