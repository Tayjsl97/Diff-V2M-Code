...
class ListConditioner(Conditioner):
    ...
    def forward(self, texts: tp.List[str], device=None) -> tp.Any:
        ...
        int_embeds = self.embedder(ints).unsqueeze(1)
...
class T5Conditioner(Conditioner):
    ...
    def __init__(
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            try:
                self.tokenizer = AutoTokenizer.from_pretrained("saved_models/t5_base")
                model = T5EncoderModel.from_pretrained("saved_models/t5_base").train(enable_grad).requires_grad_(enable_grad).to(
                    torch.float16)
            finally:
                logging.disable(previous_level)
...
class SemanticConditioner(Conditioner):
    def __init__(
        ...
        self.max_length = max_length
        self.mid_dim = 512
        if rhythm_type in ['mel','tempo']:
            self.extractor = RhythmExtractor(input_dim=dim,output_dim=16,dim=self.mid_dim)
        elif rhythm_type in ['chroma']:
            self.extractor = RhythmExtractor(input_dim=dim,output_dim=12,dim=self.mid_dim)
        elif rhythm_type in ['odf']:
            self.extractor = RhythmExtractor(input_dim=dim,output_dim=1,dim=self.mid_dim)
        self.proj_out1 = nn.Linear(dim, self.mid_dim) if (dim != self.mid_dim) else nn.Identity()
        self.proj_out2 = nn.Linear(self.mid_dim, output_dim)

    def pad_video_feat(self,video_feat):
        padded_tensors = []
        for t in video_feat:
            t = torch.from_numpy(t) if not isinstance(t, torch.Tensor) else t
            temp = torch.cat([t, torch.zeros(self.max_length - t.shape[0], t.shape[1])], dim=0)
            padded_tensors.append(temp)
        padded_tensors = torch.stack(padded_tensors)

        mask = torch.stack([
            torch.cat([torch.ones(t.shape[0]), torch.zeros(self.max_length - t.shape[0])]) for t in video_feat
        ])
        return padded_tensors, mask

    def pad_rhythm_info(self,beat):
        padded_tensors = []
        for t in beat:
            t = torch.from_numpy(np.array(t,dtype=np.float32)) if not isinstance(t, torch.Tensor) else t
            temp = torch.cat([t, torch.zeros(self.max_length - t.shape[0])], dim=0)
            padded_tensors.append(temp)
        padded_tensors = torch.stack(padded_tensors)
        return padded_tensors

    def forward(self,video_feat,class_label,scene_cut=None,pseudo_beat=None,device=None):
        if isinstance(video_feat, list) or isinstance(video_feat, tuple):
            video_feat,mask = self.pad_video_feat(video_feat)
        else:
            raise ValueError("Video feat should be a list or tuple")
        if isinstance(scene_cut, list) or isinstance(scene_cut, tuple):
            scene_cut = self.pad_rhythm_info(scene_cut)
            pseudo_beat = self.pad_rhythm_info(pseudo_beat)
        else:
            raise ValueError("Video feat should be a list or tuple")
        video_feat = video_feat.to(device)
        mask = mask.to(device)
        x = self.proj_out1(video_feat)
        if x.dtype != self.proj_out2.weight.dtype:
            x = x.to(dtype=self.proj_out2.weight.dtype)
        x = self.proj_out2(x)
        fake_rhythm_feat = self.extractor(video_feat,class_label,scene_cut,pseudo_beat,mask)
        return x.to(device),mask.to(device),fake_rhythm_feat.to(device)

class RhythmConditioner(Conditioner):
    def __init__(
            self,
            dim: int,
            output_dim: int,
    ...
    def resize_spectrogram(self,mel_spectrogram, attention, target_shape):

        target_shape = tuple(target_shape) if isinstance(target_shape, (list, tuple)) else (target_shape,)


        batch_size,m, c = mel_spectrogram.shape
        target_m, target_c = target_shape
        
        if m < target_m:
            pad_len = target_m - m
            mel_padding = torch.zeros((batch_size, pad_len, c), device=mel_spectrogram.device)
            mel_spectrogram = torch.cat([mel_spectrogram, mel_padding], dim=1)

            attn_padding = torch.zeros((batch_size, pad_len), device=attention.device)
            attention = torch.cat([attention, attn_padding], dim=1)
            m = target_m

        scale_factor_m = m // target_m
        scale_factor_c = c // target_c

        resized_spectrogram = torch.zeros((batch_size,target_m, target_c)).to(mel_spectrogram.device)
        resized_attention = torch.zeros((batch_size,target_m)).to(mel_spectrogram.device)

        for k in range(batch_size):
            mel = mel_spectrogram[k]
            attn = attention[k]

            mask_len = int(torch.sum(attn).item() // scale_factor_m)
            tmp = torch.ones(mask_len, device=attn.device)

            resized_attention[k, :mask_len] = tmp

            for i in range(target_m):
                for j in range(target_c):
                    m_start = i * scale_factor_m
                    m_end = (i + 1) * scale_factor_m
                    c_start = j * scale_factor_c
                    c_end = (j + 1) * scale_factor_c

                    sub_block = mel[m_start:m_end, c_start:c_end]
                    resized_spectrogram[k, i, j] = torch.mean(sub_block)

        return resized_spectrogram,resized_attention

    def pad_video_feat(self,video_feat):
        max_len = max(t.shape[0] for t in video_feat)

        padded_tensors = torch.stack([
            torch.cat([t, torch.zeros(max_len - t.shape[0], t.shape[1]).to(video_feat[0].device)], dim=0) for t in video_feat
        ])

        mask = torch.stack([
            torch.cat([torch.ones(t.shape[0]), torch.zeros(max_len - t.shape[0])]) for t in video_feat
        ]).to(padded_tensors.device)
        return padded_tensors, mask
    ...
    def min_max_normalize_tensor(self,x):
        x_min = x.min(dim=(1, 2), keepdim=True)[0]
        x_max = x.max(dim=(1, 2), keepdim=True)[0]

        return (x - x_min) / (x_max - x_min)

    def forward(self, video_feat, attention_mask, device=None,rhythm_type='mel'):
        if isinstance(video_feat, list) or isinstance(video_feat, tuple):
            if rhythm_type in ['mel','tempo']:
                dim=16
            elif rhythm_type in ['odf']:
                dim=1
            video_feat, attention_mask = self.pad_video_feat(video_feat)
            video_feat, attention_mask = self.resize_spectrogram(video_feat, attention_mask, (self.max_length, dim))
            video_feat = self.normalize_tensor(video_feat).to(self.proj_out2.weight.device)

        x=self.proj_out1(video_feat)
        x=self.proj_out2(x)
        return x.to(device),attention_mask.to(device),video_feat.to(device)

class ColorConditioner(Conditioner):
    def __init__(
            self,
            dim: int,
            output_dim: int,
    ...
    def pad_video_feat(self,video_feat):
        ...
        for t in video_feat:
            ...
            t = torch.from_numpy(t) if not isinstance(t, torch.Tensor) else t
            temp = torch.cat([t, torch.zeros(self.max_length - t.shape[0], t.shape[1])], dim=0)
            temp_mask = torch.cat([torch.ones(t.shape[0]), torch.zeros(self.max_length - t.shape[0])])
            padded_tensors.append(temp)
            padded_masks.append(temp_mask)
        padded_tensors = torch.stack(padded_tensors)
        mask = torch.stack(padded_masks)
        return padded_tensors, mask

    def forward(self,video_feat,device=None):
        if isinstance(video_feat, list) or isinstance(video_feat, tuple):
            video_feat,mask = self.pad_video_feat(video_feat)
        else:
            raise ValueError("Video feat should be a list or tuple")
    ...
    def forward(self, batch_metadata: tp.List[tp.Dict[str, tp.Any]], device: tp.Union[torch.device, str]) -> tp.Dict[str, tp.Any]:
        ...
        for key, conditioner in self.conditioners.items():
            ...
            for x in batch_metadata:
                ...
                class_label.append(x['class'])
                if "scene_cut" in x.keys() and x['scene_cut'] is not None:
                    scene_cut.append(x['scene_cut'])
                    pseudo_beat.append(x['pseudo_beat'])

            if key in self.pre_encoded_keys:
                output[key] = [torch.stack(conditioner_inputs, dim=0).to(device), None]
            elif isinstance(conditioner,SemanticConditioner):
                if len(scene_cut)>0:
                    x, mask_x, predict_rhythm = conditioner(conditioner_inputs,class_label,scene_cut=scene_cut,pseudo_beat=pseudo_beat,device=device)
                else:
                    x, mask_x, predict_rhythm = conditioner(conditioner_inputs, class_label,device=device)
                output[key] = (x,mask_x)
                with torch.no_grad():
                    output['fake_rhythm'] = self.conditioners['rhythm'](predict_rhythm,mask_x,device=device,rhythm_type=rhythm_type)[:2]
                output['fake_rhythm_loss'] = predict_rhythm
            elif isinstance(conditioner,RhythmConditioner):
                x,mask_x,real_rhythm = conditioner(conditioner_inputs,None,device=device,rhythm_type=rhythm_type)
                output[key] = (x, mask_x)
                output['real_rhythm_loss'] = real_rhythm
            else:
...
def create_multi_conditioner_from_conditioning_config(config: tp.Dict[str, tp.Any], pretransform=None) -> MultiConditioner:
    ...
    for conditioner_info in config["configs"]:
        ...
        elif conditioner_type == "rhythm":
            conditioners[id] = RhythmConditioner(**conditioner_config)
        elif conditioner_type == "color":
            conditioners[id] = ColorConditioner(**conditioner_config)
